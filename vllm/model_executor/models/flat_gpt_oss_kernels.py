...
def _linear_gemv_kernel(
    ...
    weight_stride_o: tl.int64,
    weight_stride_k: tl.int64,
    ...
    out_block = tl.program_id(axis=1)
    offs_o = out_block * BLOCK_O + tl.arange(0, BLOCK_O)
    offs_k = tl.arange(0, BLOCK_K)
    mask_k = offs_k < in_features

    x = tl.load(input_ptr + row * input_stride + offs_k, mask=mask_k,
                other=0.0).to(tl.float32)
    w = tl.load(
        weight_ptr + offs_o[:, None] * weight_stride_o
        + offs_k[None, :] * weight_stride_k,
        mask=(offs_o[:, None] < out_features) & mask_k[None, :],
        other=0.0,
    ).to(tl.float32)
    acc = tl.sum(w * x[None, :], axis=1)
    if HAS_BIAS:
        acc += tl.load(bias_ptr + offs_o, mask=offs_o < out_features, other=0.0)
    tl.store(
        output_ptr + row * output_stride + offs_o,
        acc,
        mask=offs_o < out_features,
    )
...
def linear_gemv(
    ...
    _linear_gemv_kernel[(input.shape[0], triton.cdiv(out_features, block_o))](
        input,
        weight,
