...
@triton.jit
def _index_qk_rope_quant_cache_weights_kernel(
    index_q_ptr,  # [num_q_tokens, n_heads, head_dim]
    index_k_ptr,  # [num_k_tokens, head_dim]
    positions_ptr,  # [num_tokens]
    cos_sin_cache_ptr,  # [max_position, rope_dim]
    index_weights_ptr,  # [num_q_tokens, n_heads]
    norm_weight_ptr,  # [head_dim]
    norm_bias_ptr,  # [head_dim]
    slot_mapping_ptr,  # [num_k_tokens]
    q_fp8_ptr,  # [num_q_tokens, n_heads, head_dim]
    scaled_weights_ptr,  # [num_q_tokens, n_heads]
    kv_cache_fp8_ptr,  # flat fp8 view of [num_blocks, block_size, cache_stride]
    kv_cache_f32_ptr,  # flat float32 view of same storage for scale writes
    num_q_tokens: tl.constexpr,
    num_k_tokens: tl.constexpr,
    ...
    if stream < n_heads:
        ...
        tl.store(
            q_fp8_ptr
            + token * q_fp8_stride0
            + stream * q_fp8_stride1
            + cols * q_fp8_stride2,
            q_vals,
            mask=mask,
        )
        route_weight = tl.load(
            index_weights_ptr + token * weights_stride0 + stream * weights_stride1
        ).to(tl.float32)
        tl.store(
            scaled_weights_ptr
            + token * scaled_weights_stride0
            + stream * scaled_weights_stride1,
            route_weight * scale * FACTOR,
        )
    else:
        k_valid_token = token < num_k_tokens
        x = tl.load(
            index_k_ptr + token * index_k_stride0 + cols * index_k_stride1,
            mask=mask & k_valid_token,
            other=0.0,
        ).to(tl.float32)
        mean = tl.sum(tl.where(mask, x, 0.0), axis=0) / head_dim
        centered = tl.where(mask, x - mean, 0.0)
        ...
        bias = tl.load(norm_bias_ptr + cols, mask=mask, other=0.0).to(tl.float32)
        vals = centered * inv_std * norm_weight + bias

        x_pair = tl.load(
            index_k_ptr + token * index_k_stride0 + rope_pair * index_k_stride1,
            mask=rope_mask & k_valid_token,
            other=0.0,
        ).to(tl.float32)
        y_pair = tl.load(
            index_k_ptr
        ...
        slot = tl.load(slot_mapping_ptr + token, mask=k_valid_token, other=-1)
        valid = (slot >= 0) & k_valid_token
        block_idx = slot // cache_block_size
        block_offset = slot - block_idx * cache_block_size
        base = block_idx * cache_block_size * cache_stride
        data_offset = base + block_offset * head_dim + cols
